...
export async function syncLiveMatches(): Promise<{
  ...
  try {
    ...
    console.log(`[CRON] Fetching fixtures for date=${today} league=${WC_2026_LEAGUE_ID}`);
    const rawResponse = await fetch(`${API_FOOTBALL_BASE}/fixtures?league=${WC_2026_LEAGUE_ID}&season=${WC_2026_SEASON}&date=${today}`, {
      headers: { "x-apisports-key": process.env.FOOTBALL_API_KEY! },
      next: { revalidate: 0 },
    });
    const rawData = await rawResponse.json();
    console.log(`[CRON] API response: status=${rawResponse.status} results=${rawData.results} errors=${JSON.stringify(rawData.errors)}`);
    const fixtures = rawData.response as APIFixture[];
